...
class TodoManager {
    ...
    return `
      <div class="todo-item ${completedClass}" data-todo-id="${todo.id}">
        ...
        <div class="todo-actions">
          <button class="btn btn-outline btn-sm edit-todo" data-todo-id="${
            todo.id
          }" title="Edit">
          ...
          <button class="btn btn-danger btn-sm delete-todo" data-todo-id="${
            todo.id
          }" title="Delete">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  updateStats() {
    ...
    if (todoCountElement) {
      ...
      switch (this.filter) {
        case "completed":
          countText = `${completedTodos} completed todo${
            completedTodos !== 1 ? "s" : ""
          }`;
          break;
        case "pending":
          countText = `${pendingTodos} pending todo${
            pendingTodos !== 1 ? "s" : ""
          }`;
          break;
        default:
